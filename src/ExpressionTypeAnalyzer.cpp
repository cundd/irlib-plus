...
ExpressionType
ExpressionTypeAnalyzer::detectExpressionType(std::string expression)
{
    trimToFirstWord(expression);

    if (ExpressionTypeAnalyzer::keywords.find(expression)
        != ExpressionTypeAnalyzer::keywords.end()) {
